...
describe('now-confirm-dialog tests', function () {
	...
	it('should have custom background and colors defined for cancel and confirm buttons', function () {
		let confirmBackground = (<any>window).ShadyCSS.getComputedStyleValue(element, '--now-confirm-dialog-confirm-button-background');
		let confirmColor = (<any>window).ShadyCSS.getComputedStyleValue(element, '--now-confirm-dialog-conform-button-color');;
		let cancelBackground = (<any>window).ShadyCSS.getComputedStyleValue(element, '--now-confirm-dialog-cancel-button-background');;
		let cancelColor = (<any>window).ShadyCSS.getComputedStyleValue(element, '--now-confirm-dialog-cancel-button-color');;
		chai.expect(confirmBackground).equals('purple');
		chai.expect(confirmColor).equals('black');
		chai.expect(cancelBackground).equals('pink');
		chai.expect(cancelColor).equals('black');
	});

	/* it('should have dynamic text and title', function () {
		(<any>element).dialogText = 'This is custom text';
		(<any>element).dialogTitle = 'Custom Title';
		element.open(null, null);
	...
	/* it('should have custom text for the confirm and cancel buttons', function () {
		...
		let confirmButton = <any> Polymer.dom(element.root).querySelector('.confirmButton');
		(<any>element).confirmButtonText = 'YUP';
		(<any>element).cancelButtonText = 'NOPE';
